...
`record-verification` is a one-shot, host-scoped reconciliation checkpoint for a terminal task. It requires at least one declared `requiredVerification` command and at least one matching outcome. The runtime serializes it with workspace admission, refuses it while another writer is active, and compares the completion snapshot to the current checkout. Every observed change must remain inside the original `scope.include` and outside `scope.exclude`; an empty include list permits no verification-time drift. Git-state or changed-path-overflow markers fail closed. Only then may the runtime capture `verificationContextManifest` and bind an explicit continuation to that exact snapshot. The record carries authority `host_asserted`: it states what the host reports and preserves a bounded state checkpoint, but it does not prove that a particular command caused the intervening workspace changes.
