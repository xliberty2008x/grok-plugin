...
- Independent Claude Code and remaining platform provider qualification beyond the exact issue #25 macOS/Codex evidence.
...
Task authors MUST declare or isolate ignored outputs that a host verification command is expected to rewrite. `verificationGeneratedPaths` is a bounded list (at most 64) of repository-relative files or directories. During `record-verification` only, those paths and their descendants are excluded from the verification ignored-worktree identity together with exact `.pytest_cache` and `__pycache__` path components. The declaration is not inferred from command strings. Isolating generated output outside the workspace, or into a declared directory, is equivalent. Ordinary completion and explicit resume still compare the full ignored digest, including undeclared `dist/**` and `node_modules/**`.

The runtime serializes `record-verification` with workspace admission, refuses it while another writer is active, and compares the completion snapshot to the current checkout. Only for this comparison, ignored paths with an exact `.pytest_cache` or `__pycache__` path component, and paths declared in `verificationGeneratedPaths`, are excluded; similarly named components and undeclared ignored files remain observable. Missing verification-only identity fields fall back to the full ignored-worktree comparison. HEAD, index, Git metadata, dirty-path overflow, and non-attributable ignored drift keep their existing fail-closed behavior. Every observed change must remain inside the original `scope.include` and outside `scope.exclude`; an empty include list permits no verification-time drift. The runtime then stores the exact full `verificationContextManifest`, including cache identity, so ordinary completion checks and explicit continuation resume remain bound to full workspace state. The record carries authority `host_asserted`: it states what the host reports and preserves a bounded state checkpoint, but it does not prove that a particular command caused the intervening workspace changes.
...
- Document residual limitations honestly: macOS child-process network isolation is not enforced by Grok; write-scope violations inside isolated execution worktrees are detected after the fact and violating artifacts are not integration-ready; issue #25 operational qualification is bound to exact source `e89272f` and evidence head `ef2dd03`; independent Claude Code and remaining platform qualification are still open; July 13 evidence does not qualify the changed worktree.
...
- A historical authenticated macOS Grok Build 0.2.99 matrix passed on July 13, 2026 and is retained in `tests/e2e-results/` as prior evidence for an earlier contract (profile contract v2 / pre-hardening). That result does **not** qualify the current worktree's TaskEnvelope, schema-3, profile-v3, no-terminal write, credential-revocation, or host-control-plane changes. Issue #25 separately reached `operational_e2e_complete` on exact source `e89272f` and evidence head `ef2dd03` with official Grok Build 0.2.112, including a natural installed-Codex flow and host verification. That evidence is scoped to the recorded macOS/Codex Worker Broker path and does not qualify the dual-host prerelease for release. Linux provider execution remains authenticated-provider-unverified. Windows provider execution and process control are unsupported in v0.3 until a trustworthy process-start identity and authenticated lifecycle evidence exist.
- Release notes MUST identify 0.2.99 as the enforced compatibility floor and 0.2.112 as the exact issue #25 operational qualification endpoint, without claiming other versions, hosts, platforms, or the dual-host prerelease are release-qualified.
