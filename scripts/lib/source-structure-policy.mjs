...
export const SOURCE_STRUCTURE_POLICY_DIGEST = "3a67d5a814ff4dccf1c9e969b729c25b655f69a645dd933322c329782733cfa5";
...
function currentPolicyProjection(config) {
  return canonicalPolicyValue({
    baseline: {
      date: config?.baseline?.date,
      initialDigest: config?.baseline?.initialDigest,
      revision: config?.baseline?.revision
    },
    budgets: config?.budgets,
    capCycleComponents: config?.capCycleComponents,
    capOrdinalFragments: config?.capOrdinalFragments,
    dispositions: config?.dispositions,
    extensions: config?.extensions,
    facadePaths: config?.facadePaths,
    legacyDebt: config?.legacyDebt,
    maxPhysicalLineBytes: config?.maxPhysicalLineBytes,
