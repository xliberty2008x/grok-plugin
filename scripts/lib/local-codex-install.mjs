import os from "node:os";
import path from "node:path";
import process from "node:process";
import { spawnSync } from "node:child_process";

import {
  createPluginInventory,
  describeInventoryDifference,
  digestInventory,
  isPathInside,
  canonicalPath
} from "./plugin-inventory.mjs";
import {
  assertReceiptMatchesIdentity,
  collectInstallIdentity,
  qualificationReceiptPath,
  readQualificationReceipt
} from "./qualification-receipt.mjs";

const PLUGIN_ID = "grok@grok-companion";
const MARKETPLACE = "grok-companion";

function fail(message, details = "") {
  const suffix = details.trim() ? `\n${details.trim()}` : "";
  throw new Error(`${message}${suffix}`);
}

function runChecked(command, args, { cwd, env = process.env, timeout = 180_000, run } = {}) {
  const runner = run ?? spawnSync;
  const result = runner(command, args, {
    cwd,
    env,
    encoding: "utf8",
    shell: false,
    timeout,
    maxBuffer: 32 * 1024 * 1024
  });
  if (result.error) fail(`Could not run ${command} ${args.join(" ")}.`, result.error.message);
  if (result.status !== 0) {
    fail(
      `${command} ${args.join(" ")} exited with status ${result.status}.`,
      [result.stdout, result.stderr].filter(Boolean).join("\n")
    );
  }
  return result;
}

function parseJson(result, label) {
  try {
    return JSON.parse(result.stdout);
  } catch {
    fail(`${label} did not return valid JSON.`, result.stdout || result.stderr);
  }
}

/**
 * Fast consumer install. Never runs the repository suite.
 */
export function runLocalCodexInstall(options = {}) {
  const root = options.root;
  if (typeof root !== "string" || root.length < 1) {
    throw new Error("install requires a repository root.");
  }
  if (process.platform === "win32" && options.allowWindows !== true) {
    fail("codex:install currently requires the POSIX nonblocking PTY gate; run it on macOS or Linux.");
  }

  const write = options.write ?? ((text) => process.stdout.write(text));
  const run = options.run ?? spawnSync;
  const codexBin = options.codexBin ?? process.env.CODEX_BIN ?? "codex";
  const receiptPath = options.receiptPath ?? qualificationReceiptPath(root);

  const spawned = [];
  const trackedRun = (command, args, extra) => {
    spawned.push([command, ...args].join(" "));
    return run(command, args, extra);
  };

  write("[1/3] Verifying the qualification receipt...\n");
  const identity = collectInstallIdentity(root);
  const receipt = readQualificationReceipt(receiptPath);
  assertReceiptMatchesIdentity(receipt, identity);
  if (spawned.some((entry) => entry.includes("run check") || entry.includes("test:deterministic"))) {
    fail("Install path must not run the repository qualification suite.");
  }

  write("[2/3] Refreshing the Codex cache...\n");
  const marketplacePayload = parseJson(
    runChecked(codexBin, ["plugin", "marketplace", "list", "--json"], {
      cwd: root,
      timeout: 30_000,
      run: trackedRun
    }),
    "codex plugin marketplace list"
  );
  const marketplace = marketplacePayload.marketplaces?.find((entry) => entry.name === MARKETPLACE);
  if (!marketplace?.root) fail(`Codex marketplace ${MARKETPLACE} is not configured.`);
  const expectedMarketplaceRoot = canonicalPath(root, "Repository root");
  const actualMarketplaceRoot = canonicalPath(marketplace.root, `Codex marketplace ${MARKETPLACE}`);
  if (actualMarketplaceRoot !== expectedMarketplaceRoot) {
    fail(
      `Codex marketplace ${MARKETPLACE} points at a different checkout.`,
      `expected: ${expectedMarketplaceRoot}\nactual:   ${actualMarketplaceRoot}`
    );
  }

  const installedPayload = parseJson(
    runChecked(codexBin, ["plugin", "add", PLUGIN_ID, "--json"], {
      cwd: root,
      timeout: 60_000,
      run: trackedRun
    }),
    "codex plugin add"
  );
  if (!installedPayload.installedPath) fail("Codex did not report the installed cache path.");

  write("[3/3] Comparing the installed snapshot with the qualified source...\n");
  const pluginList = parseJson(
    runChecked(codexBin, ["plugin", "list", "--json"], {
      cwd: root,
      timeout: 30_000,
      run: trackedRun
    }),
    "codex plugin list"
  );
  const installedRecord = pluginList.installed?.find((entry) => entry.pluginId === PLUGIN_ID);
  if (!installedRecord?.installed || !installedRecord.enabled) {
    fail(`${PLUGIN_ID} is not installed and enabled after refresh.`);
  }
  if (installedRecord.version !== identity.version) {
    fail(`Installed version ${installedRecord.version} does not match source version ${identity.version}.`);
  }

  const installedRoot = canonicalPath(installedPayload.installedPath, "Installed plugin cache");
  const codexHome = canonicalPath(
    options.codexHome ?? process.env.CODEX_HOME ?? path.join(os.homedir(), ".codex"),
    "Codex home"
  );
  const expectedCacheRoot = path.join(codexHome, "plugins", "cache", MARKETPLACE, "grok");
  if (!isPathInside(expectedCacheRoot, installedRoot)) {
    fail("Codex reported an installed path outside the expected plugin cache.", installedRoot);
  }
  const collectInstalled = options.createInstalledInventory ?? createPluginInventory;
  const installed = collectInstalled(installedPayload.installedPath);
  if (!installed) fail("Could not inventory the installed plugin cache.");
  const differences = describeInventoryDifference(identity.sourceEntries, installed);
  if (differences.length > 0) {
    fail("Installed Codex snapshot does not match the source plugin.", differences.join("\n"));
  }
  if (digestInventory(installed) !== identity.sourceDigest) {
    fail("Installed snapshot digest does not match the qualified source digest.");
  }
  if (spawned.some((entry) => /\bnpm\b.*\bcheck\b/.test(entry) || entry.includes("test:deterministic"))) {
    fail("Install path must not run the repository qualification suite.");
  }

  write([
    "Local Codex plugin install passed.",
    `  plugin:  ${PLUGIN_ID}`,
    `  version: ${identity.version}`,
    `  files:   ${identity.fileCount}`,
    `  digest:  ${identity.sourceDigest}`,
    `  cache:   ${installedRoot}`,
    "Start a new Codex task before testing the update."
  ].join("\n") + "\n");

  return {
    identity,
    receipt,
    installedRoot,
    spawned
  };
}
