...
test("review worker provisionally authorizes before workerProcess is published", {
  ...
  // Foreground/background share durable job state; this exercises the worker
  // entry both launch modes use, with the parent deliberately withholding
  // workerProcess past the old pure-poll window.
  const root = initRepo();
  // Seed workspace first (empty-target path is fast), then dirty the tree for the
  // provisional worker under test. Do not put headlessDelayMs on seedWorkspace.
  ...
  // Parent never publishes workerProcess. Child may self-bind inside execute after
  // provisional auth; wait longer than the old pure-poll window to prove that path.
  ...
  // Child self-bind is fine; parent never wrote an initial identity in this fixture.
