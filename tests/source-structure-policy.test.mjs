...
const EVIDENCE_FACADE_RELATIVE = "scripts/lib/worker-broker-evidence.mjs";
const EVIDENCE_FACADE = path.join(ROOT, EVIDENCE_FACADE_RELATIVE);
const EVIDENCE_PUBLIC_MODULES = Object.freeze([
...
function localStaticClosure(seed) {
  const pending = [seed];
  ...
  while (pending.length > 0) {
    ...
    for (const specifier of parseSourceStructure(source, relative).specifiers) {
...
test("configuration fails closed on weakened boundaries, wildcard paths, and unsorted function vectors", () => {
  ...
  wrongFacadeFunctionBudget.budgets.facade.functionLines = 999_999;
  assert.ok(validateSourceStructurePolicy(wrongFacadeFunctionBudget)
    .some((message) => /canonical budgets/u.test(message)));

  const emptyRoots = policy();
  emptyRoots.roots = [];
  assert.ok(validateSourceStructurePolicy(emptyRoots).some((message) => /canonical set/u.test(message)));

  const missingExtension = policy();
  missingExtension.extensions = [".js", ".mjs"];
  assert.ok(validateSourceStructurePolicy(missingExtension).some((message) => /canonical set/u.test(message)));

  const weakenedLineLimit = policy();
  weakenedLineLimit.maxPhysicalLineBytes += 1;
  assert.ok(validateSourceStructurePolicy(weakenedLineLimit)
    .some((message) => /maxPhysicalLineBytes/u.test(message)));
...
test("immutable digest pins initial debt and topology while caps may only shrink", () => {
  ...
  const workerMutation = "plugins/grok/scripts/lib/worker-mutation.mjs";
  const activeDebt = "plugins/grok/scripts/lib/worker-owner-lifecycle.mjs";
  ...
  assert.equal(config.baseline.date, "2026-08-01");
  assert.equal(config.baseline.revision, REVISION);
  assert.equal(config.initialDebt[workerMutation].initialLines, 11269);
  assert.equal(config.initialDebt[workerMutation].functions.length, 14);
  assert.equal(config.legacyDebt[workerMutation], undefined);
  assert.equal(config.dispositions[workerMutation], undefined);
  assert.ok(config.facadePaths.includes(workerMutation));

  const changedInitial = structuredClone(config);
  changedInitial.initialDebt[workerMutation].initialLines += 1;
  assert.ok(validateSourceStructurePolicy(changedInitial, {
    expectedInitialDigest: SOURCE_STRUCTURE_INITIAL_DIGEST
  }).some((message) => /initialDigest/u.test(message)));

  const raisedCap = structuredClone(config);
  raisedCap.legacyDebt[activeDebt].lineCap += 1;
  assert.ok(validateSourceStructurePolicy(raisedCap, {
    expectedInitialDigest: SOURCE_STRUCTURE_INITIAL_DIGEST
  }).length > 0);

  const lowerCap = structuredClone(config);
  lowerCap.legacyDebt[activeDebt].lineCap -= 1;
  ...
  delete removedCap.legacyDebt[activeDebt];
  ...
  const reopenedCap = structuredClone(config);
  reopenedCap.legacyDebt["plugins/grok/scripts/lib/deep-research.mjs"].lineCap += 1;
  ...
  rewrittenProvenance.baseline.date = "2026-08-02";
...
test("excluded dependency directories and replaced scan roots fail closed", () => withRepository((root) => {
  write(root, "scripts/main.mjs", "import './node_modules/hidden.mjs';\n");
  write(root, "scripts/node_modules/hidden.mjs", "export const hidden = true;\n");
  const excluded = evaluateSourceStructure({ root, config: policy(), mode: "ratchet" });
  assert.ok(codes(excluded, "error").has("excluded-source-edge"));
...
test("checked-in evidence facade is bounded, one-way, and keeps immutable history", () => {
  const config = loadSourceStructurePolicy({ root: ROOT });
  const result = evaluateSourceStructure({ root: ROOT, config });
  assert.ok(config.facadePaths.includes(EVIDENCE_FACADE_RELATIVE));

  const facadeSource = fs.readFileSync(EVIDENCE_FACADE, "utf8");
  const facadeStructure = parseSourceStructure(facadeSource, EVIDENCE_FACADE_RELATIVE);
  assert.ok(physicalLineCount(facadeSource) <= config.budgets.facade.fileLines);
  assert.deepEqual(
    facadeStructure.specifiers.filter((specifier) => specifier.startsWith("./")).sort(),
    EVIDENCE_PUBLIC_MODULES
      .filter((file) => !file.endsWith("-proof.mjs"))
      .map((file) => `./${path.basename(file)}`).sort()
  );

  const publicClosure = localStaticClosure(EVIDENCE_FACADE).filter((file) => (
    file.startsWith("scripts/lib/worker-broker-evidence-")
  ));
  assert.deepEqual(publicClosure, EVIDENCE_PUBLIC_MODULES);
  assert.equal(
    publicClosure.includes("scripts/lib/worker-broker-evidence-authority.mjs"),
    false
  );
  assert.equal(
    publicClosure.includes("scripts/lib/worker-broker-evidence-protected-trust.mjs"),
    false
  );
  for (const relative of EVIDENCE_PUBLIC_MODULES) {
    const source = fs.readFileSync(path.join(ROOT, relative), "utf8");
    const structure = parseSourceStructure(source, relative);
    assert.ok(physicalLineCount(source) <= config.budgets.tooling.fileLines, relative);
    assert.equal(
      structure.functions.some(
        (entry) => entry.lines > config.budgets.tooling.functionLines
      ),
  ...
  assert.equal(
    [...result.errors, ...result.warnings].some((entry) => (
      entry.code === "reverse-facade-import"
      && /worker-broker-evidence/u.test(`${entry.file} ${entry.message}`)
    )),
    false
  );
  assert.deepEqual(config.initialDebt[EVIDENCE_FACADE_RELATIVE], {
    category: "tooling",
    functions: [
      { initialLines: 390, key: "function:promotePhaseOneSignedReviewInternal#1" },
      { initialLines: 1033, key: "function:validateEvidenceRecord#1" }
    ],
    initialLines: 9502
  });
  assert.equal(Object.hasOwn(config.legacyDebt, EVIDENCE_FACADE_RELATIVE), false);
  assert.equal(Object.hasOwn(config.dispositions, EVIDENCE_FACADE_RELATIVE), false);
  assert.equal(config.baseline.initialDigest, SOURCE_STRUCTURE_INITIAL_DIGEST);
  assert.equal(sourceStructureInitialDigest(config), SOURCE_STRUCTURE_INITIAL_DIGEST);
});
...
test("checked-in observe baseline exactly covers all current file and function debt", () => {
  ...
  for (const [file, initial] of Object.entries(config.initialDebt)) {
    ...
    if (!cap) {
      assert.equal(debtPaths.includes(file), false);
      continue;
    }
  ...
  const driftCodes = new Set([
    ...
    "reverse-facade-import", "stale-file-cap", "stale-function-cap",
