...
| 2 — Mailbox/context/roles | `implemented_unverified` partial production | Idempotent mailbox records, follow-up foundations, explicit ambiguity states, broker-built canonical ContextPacket, immutable explorer role/profile/tool policy, body-free Context Receipt, exact effective-prompt and provider-lineage binding, restart/tamper rejection, and legacy-dispatch compatibility | Production send has no ACP consumer; follow-up is not dispatchable; ordering, autonomous crash settlement, durable host-action grant/deny, non-explorer admission, and a Phase 2 proof producer are missing |
...
| --- | ---: | --- |
| Full roadmap source implementation | `43%` — `█████████░░░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0; committed Q2 plus source-complete P2.1 support 20 of 46 non-deferred deliverables (48 total IDs minus P4-D6/P4-D7 deferred) |
...
| Phase 2 | `4.00 / 8` | ContextPacket and Context Receipt are production-wired for the explorer vertical; ACP delivery/follow-up, host action, and the fixed producer remain |
...
| **Total** | **`20.00 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P2 | `D1=0.25, D2=0.5, D3=0.25, D4=1, D5=0.25, D6=0.5, D7=0.25, D8=1` |
...
| P2-D6 | Immutable explorer/implementer/reviewer/security/test roles | `implemented_unverified` explorer production path; other exact role identities remain unadvertised | RuntimeRolePolicy binds role digest, write bit, exact provider profile/version/agent bytes, and complete allowed/denied provider-tool ID digests; launch preparation rechecks the actual prompt/profile binding | Add durable owner-only future-admission grants and role-specific public admission before reviewer/security/test or implementer is advertised |
...
| A-32 Stable spawn response and installed runner | Commit `858ceea` returns the exact transaction-time untrusted admission handle and records a private schema-4 response witness instead of replacing the response with a moving post-dispatch reread. Commit `fda21c2` adds exact witness/launch-contract/public-private binding plus the fixed installed completion and reconnect/replay/cancellation runner. | Pre-commit integrated checks: 130/130 on current Node and 130/130 on Node 18.18.2; focused runner/contract and independent-review batches passed on both runtimes; two fresh native reviews approved after the production `final.report` chronology was corrected. These moving-tree checks do not replace the clean exact-source replay. | Run the clean committed 27-file Phase 1 gate and full repository gate, then execute the authenticated installed runner. Keep the receipt provisional because later Phase 2–5 source work will supersede it |
...
- The first exact post-commit repository gate on `f306ddd` passed validation but correctly failed the deterministic gate at 723 passed / 18 failed / 0 skipped / 0 TODO. Raw replay separated three genuine launch-outbox compatibility gaps from a process-harness timeout cascade. The follow-up commit containing this checkpoint binds the canonical packet/policy prompt in the path-alias fixture, models the exact pre-P2.1 request and provider-profile shape, rejects every post-v1 context/profile witness, atomically upgrades an exact legacy profile before v2 authorization, raises only bounded test-process budgets, and prevents an earlier readiness failure from leaving an unobserved child-promise rejection. Focused follow-up proof passed launch outbox 26/26, process-boundary batch 47/47, runtime 59/59, mutation 35/35, and the two anti-downgrade cases 2/2; fresh static native validation is P0/P1-clear. The authoritative post-commit repository gate must still pass before P2.1 closeout.
- P2.2 is next: persist broker-owned host-action requests and exact owner-only grant/deny decisions that apply only to future child/follow-up admission; never rebind the current worker.
...
| P2.1 | **Source-complete, qualification pending:** broker-built ContextPacket, immutable role/profile/tool policy, and body-free Context Receipt bound to the exact provider prompt, canonical root lineage, manifest, and launch preparation; exact pre-P2.1 dispatch/profile migration upgrades atomically and rejects mixed-era witnesses | Stable 138/138 focused gate with zero skip/TODO; launch-outbox 26/26; process-boundary 47/47; runtime 59/59; mutation 35/35; delayed-stdin legacy compatibility; deleted-binding anti-downgrade before execution and after authorization; generation-2 repair/error/cancellation; 22-case hostile context matrix; schema/runtime parity; clean diff; fresh native P0/P1-clear validation; exact post-commit repository rerun remains the closeout gate |
| P2.2 | Immutable runtime role/profile/tool binding plus durable owner-only host-action grant/deny requests | Role/profile/tool drift, forged role, foreign/replayed grant, restart, resume mismatch, and hidden write-role tests |
| P2.3 | Same-session follow-up enters the normal durable launch outbox and resumes exactly one parent provider session | Commit-before-launch, replay/restart exactly-once launch, same-lineage session, context/profile drift, cancellation-window, and no-prompt-replay tests |
| P2.4 | Ordered ACP mailbox pump with attempt-bound capability, acknowledgement-bound `delivered`, and non-retried `delivery_unknown` | Multi-message ordering, duplicate/missing acknowledgement, every crash window, terminal race, private-body cleanup, and real-provider capability probe |
...
9. [Client, stable witness, and installed runner complete; authenticated proof pending] Commits `0cafff7`, `858ceea`, and `fda21c2` supply the bounded strict MCP STDIO client, exact transaction-time spawn witness, and the opt-in installed runner for exact bytes, seven tools, authenticated completion, MCP reconnect without duplicate launch, idempotent cancellation, repository immutability, session deletion, and runner-owned cleanup. The next action is the early live execution, not more runner design.
...
12. [In progress: Q1, Q2, and P2.1 source boundaries complete; P2.2 next] Complete every remaining source-changing prerequisite: Phase 2–5 runtime/features and fixed deterministic producers; private aggregate producer/publication boundary; aggregate full-scope and canonical dual-host artifact binding. Q2's synthetic protected Docker gate and native review do not replace a real final-source issuer attestation. A local subagent report or structural receipt is non-qualifying. Generic writers must remain unable to mint or link qualification. Generate no final record or receipt while these source surfaces are still moving.
