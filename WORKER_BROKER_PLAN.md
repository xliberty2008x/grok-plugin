...
| 3 — Worktrees/artifacts | `implemented_unverified` foundations plus official ACP protocol seam | Control-workspace identity, shared state, clean-parent fingerprinting, managed-worktree and artifact validation helpers, fenced `ExecutionBinding`/provisioning journal contracts, and a strict official Grok ACP create/remove/session-close adapter committed at `03afdf6` | The adapter is live-proven only as a supporting boundary. The broker does not yet admit, provision, launch, integrate, or clean a write worker through that adapter, so P3-V lifecycle qualification remains `0%` |
...
| --- | ---: | --- |
| Full roadmap source implementation | `52%` — `██████████░░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0. Current score is `24.00 / 46`; commit `03afdf6` earns only protocol/helper credit because Worker Broker does not call it yet |
...
| P3-V write-source implementation | `20%` — `████░░░░░░░░░░░░░░░░` | The official ACP seam and independent host-side create/remove proof are complete. Durable write admission, promotion-gated provisioning, ready-only provider launch, one-file artifact/integration, and broker cleanup/recovery are not yet wired |
...
| Phase 3 | `2.00 / 8` | Identity/binding/artifact helpers plus the reviewed and live-proven official ACP protocol seam exist without a production write lifecycle |
...
| **Total** | **`24.00 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P3 | `D1=0.75, D2=0.25, D3=0.25, D4=0.25, D4b=0, D5=0.25, D6=0.25, D7=0` |
...
| P3-P2 — durable write admission | `next` | A separate conditionally advertised `worker_spawn_write` commits immutable `ExecutionBinding` plus `planned` provisioning state; no provider dispatch authorization exists before `ready`; exact replay returns the same worker | State/idempotency/dirty-parent/foreign-owner negatives; inspect the private job to prove no dispatch outbox, worker authorization, or process identity exists before provisioning |
...
| P3.1 | Durable private `ExecutionBinding` plus crash-safe exact official-ACP worktree provisioning; dirty control source fails closed and provider executes only in the bound execution root | Contract is frozen in `SPEC.md`: job-first/effect-second admission, exact clean base/fingerprint, canonical control/expected-execution/common-directory identity, scope/role/profile/context binding, fenced `planned → provisioning → ready` journal, no dispatch authority before `ready`, binding digest through every launch boundary, digest-only public projection, and fail-closed adoption/cleanup. Commit `03afdf6` closes only the official protocol seam. Prove before/after-provision crash recovery, orphan cleanup, symlink/base/control-root mismatch, dirty parent, and zero parent-mutation through the production broker |
