...
| 3 — Worktrees/artifacts | `implemented_unverified` foundations plus official ACP protocol seam and internal write admission | Control-workspace identity, shared state, clean-parent fingerprinting, managed-worktree and artifact validation helpers, fenced `ExecutionBinding`/provisioning journal contracts, a strict official Grok ACP create/remove/session-close adapter committed at `03afdf6`, and an MCP-hidden durable write-plan admission committed at `65c6fba` | The adapter is live-proven only as a supporting boundary and write admission stops honestly at `planned` with no launch authority. The broker does not yet provision, launch, integrate, or clean a write worker through that adapter, so P3-V lifecycle qualification remains `0%` |
...
| --- | ---: | --- |
| Full roadmap source implementation | `53%` — `███████████░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0. Current score is `24.50 / 46`; commit `65c6fba` adds durable admission-only credit, but Worker Broker still does not call the official ACP provisioner |
...
| P3-V write-source implementation | `40%` — `████████░░░░░░░░░░░░` | The official ACP seam, independent host-side create/remove proof, and durable internal write admission are complete. Promotion-gated provisioning, ready-only provider launch, one-file artifact/integration, and broker cleanup/recovery are not yet wired |
| P3-V lifecycle qualification | `0%` — `░░░░░░░░░░░░░░░░░░░░` | Two real official ACP spikes prove the external boundary, but neither entered through installed Worker Broker MCP. Qualification starts only when one broker-owned lifecycle crosses admission → real provider edit → result → integration → restart/cancel → cleanup |
...
| Phase 3 | `2.50 / 8` | Identity/binding/artifact helpers, the reviewed and live-proven official ACP seam, and fail-closed durable write admission exist without a production provisioning/launch lifecycle |
...
| **Total** | **`24.50 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P3 | `D1=0.75, D2=0.25, D3=0.5, D4=0.5, D4b=0, D5=0.25, D6=0.25, D7=0` |
...
| 4. Verified `ExecutionBinding` for writes | `admission_only_implemented_unverified`: internal admission now requires a distinct composite write-lifecycle capability, commits an immutable binding plus revision-0 planned journal, rechecks the exact clean parent, and exposes no dispatch/provider authority. The MCP write tool remains hidden and runtime execution is not enabled | Complete the official provisioner and atomic ready-only launch path, then run the Phase 3 write vertical; no public write capability may be advertised before this exits |
...
| ID | Deliverable | Current state | Artifact or proof | Remaining work |
| --- | --- | --- | --- | --- |
| P3-D1 | Stable `controlWorkspaceId`, `controlRoot`, and `executionRoot` | `implemented_unverified` admission subset | Commit `65c6fba` persists the private expected execution-root binding; workspace/control-identity and worktree helper tests pass | Provision and independently verify the actual execution root, add its digest-only public projection, and bind production runtime to it |
...
| P3-D3 | Host-owned worktree creation from exact base | `implemented_unverified` admission subset | Commit `65c6fba` commits job-first `ExecutionBinding` plus revision-0 `planned` journal; commit `03afdf6` supplies the strict official ACP seam | Wire the crash-safe promotion-gated provisioner, response-loss adoption, launch split, retention, and cleanup lifecycle |
| P3-D4 | Clean-parent fingerprint and dirty-parent rejection | `implemented_unverified` admission subset | Write admission rejects dirty or linked origins, binds exact HEAD/tree/index-security fingerprint, and rechecks it on replay | Repeat the fingerprint and tracked/untracked/ignored/binary/symlink/mode gates through official provisioning and explicit integration |
...
- The accepted Phase 3 pivot reuses the official Grok Build ACP worktree extensions. Commit `03afdf6` contains the strict protocol seam; focused verification passed 62/62, provider regression passed, a fresh native rereview is clear, and live operation `aac84b87-2a3c-48b5-9d6e-111555d0f419` proved exact clean create/remove against Grok `0.2.111`. Commit `65c6fba` then closed P3-P2: internal MCP-hidden admission commits an immutable write `ExecutionBinding` plus planned journal, exact replay and cross-process racing converge on one worker, dirty/linked/hidden-index drift fails closed, and no dispatch, authorization, provider process, or host-action authority exists before ready. The source-identical combined gate passed 101/101, the added cross-process case passed 1/1, validation/diff checks passed, and fresh native review approved. These are supporting source checks, not Worker Broker lifecycle qualification. The next gate is the promotion-gated official provisioner.
...
The project now reuses the open-source Grok Build harness for worktree mechanics instead of extending the superseded custom `git worktree` runtime. Worker Broker remains the authority for identity, policy, durable state, verification, integration approval, and cleanup proof; the official Grok ACP extensions are the executor for create/remove and the existing official ACP session lifecycle remains the executor for model work.

```text
MCP write admission
  → durable broker binding and planned journal
  → official _x.ai/git/worktree/create
  → independent broker Git/path/base/parent verification
  → atomic ready plus provider-dispatch authority
  → existing session/new and session/prompt in the verified worktree
  → broker artifact and parent verification
  → narrowly authorized official apply
  → host check
  → official session close and worktree remove
  → independent absence proof
```

Source and runtime anchors:

- Official source checkout: `/private/tmp/xai-grok-build-reference` at `6e386420825bd44ae648c63e7c8cba12fcec9401`.
- Installed runtime: Grok Build `0.2.111` (`94172f2aa4e5`), binary resolved during the adapter probe as `/Users/cyrildubovik/.grok/downloads/grok-0.2.111-macos-aarch64`.
- Official request contract: explicit `copyMode:"clean"`, `worktreeType:"git"`, exact `gitRef`, caller-selected operation ID, and exact destination path. Defaults are not accepted as policy.
- Headless `--worktree --worktree-ref` is not the production path: the open-source headless flow drops the requested ref and opens `session/new` at the parent cwd. The raw official ACP extension path is therefore required.
- Official `apply` is sequential and non-transactional. The first vertical may use `overwrite` only for one existing tracked regular text file after exact parent, scope, mode, symlink, and byte-level preflight. The earlier simple-line `merge` probe produced a false conflict. Generalized multi-file integration remains out of scope until after P3-V.

Supporting live evidence already obtained:

| Evidence | Outcome | What it proves | What it does not prove |
| --- | --- | --- | --- |
| Raw official ACP spike, operation `c3c93fc0-07f3-4eee-9748-dd5dbb7da6a3`, ACP session `019f95fd-7d0f-7493-8a5c-ce187d513e04` | `passed` | Official create/status → real `grok-4.5` prompt/edit → clean parent → one-file overwrite apply → session close → remove works end to end in a disposable repository | No Worker Broker admission, durable journal, restart, cancellation, installed plugin, or host-owned evidence |
| Production adapter live probe, operation `e051cb6e-fdd6-4425-91e9-a285eae76c47` | `passed` | The uncommitted adapter spoke the installed wire contract and independent Git checks proved exact clean create/remove | Superseded by the reviewed final adapter revision |
| Reviewed adapter live probe, operation `aac84b87-2a3c-48b5-9d6e-111555d0f419`, base `577af6581c1b0694f89c0b10118ba6839c64a0f7` | `passed` | Commit `03afdf6` creates the exact registered clean worktree at the pinned commit, keeps the parent clean, removes the exact path, and leaves neither directory nor registration | Supporting boundary proof only; P3-V lifecycle qualification remains `0%` |

Adapter slice `03afdf6` is source-complete for its bounded contract:

- `AcpClient` publishes validated generic notifications without breaking existing `update`/`unknown` behavior.
- `GrokWorktreeAcp` uses only underscore-prefixed wire methods, unwraps the official nested result envelope, installs the status listener before create dispatch, correlates exact operation/path/commit, accepts the valid `created` + `exists` replay race, and fails closed on malformed/error/cancel/timeout/transport-close states.
- Remove requires exactly one identifier; a dry run must report `removed:false`; a real removal must report `removed:true`; an explicit worktree path must equal `resolvedPath`.
- Focused host verification passed `62/62`; provider regression passed; `git diff --check` passed; fresh native rereview reports no remaining finding.
- Grok implementation job `task-dd3a6218ce84f69f720f8e88` remains terminal `failed` with `E_PROTOCOL: Internal error`. Its separate host-verification checkpoint records the focused command as passed; it does not rewrite the provider outcome.

The pre-pivot custom runtime is preserved reversibly as stash `pre-pivot custom worker runtime (superseded by official ACP harness)`. It is not production input and must not be restored wholesale. Reuse only individually reviewed broker-authority concepts such as immutable admission identity or fencing.

#### Pivot execution slices, deliverables, and done checks

| Slice | State | Expected deliverable | Main-agent completion check and retained proof |
| --- | --- | --- | --- |
| P3-P1 — official protocol seam | `source_complete_supporting_live_pass` at `03afdf6` | Strict adapter over the existing ACP transport for create/status/remove/session-close | Focused command passes; fresh native review is clear; live operation `aac84b87…` proves exact create/remove; commit and command are recorded here and in issue #25 |
| P3-P2 — durable write admission | `source_complete` at `65c6fba`; qualification pending | An internal MCP-hidden admission seam commits immutable `ExecutionBinding` plus `planned` provisioning state under a distinct write-lifecycle capability; no provider dispatch authorization exists before `ready`; exact and cross-process replay return the same worker | Source-identical combined gate 101/101 plus cross-process 1/1; state/idempotency/schema-v5 corruption/dirty-parent/linked-origin/hidden-index/foreign-owner negatives; private-job inspection proves no dispatch outbox, worker authorization, process/session, prompt, resume, or host-action authority exists before provisioning; fresh native review approved |
| P3-P3 — promotion-gated official provisioner | `pending` | A short-lived authenticated controller performs official create, independently verifies exact registered path/base/common-dir and unchanged parent, records a bounded official receipt, and atomically promotes `planned → provisioning → ready` | Crash before/after create, response-loss adoption, mismatched/dirty/foreign path, lease reclaim, provider exit, and official cleanup negatives; one live provider-backed create/remove through the production module |
| P3-P4 — ready-only provider and artifact path | `pending` | Existing provider lifecycle launches only from the verified execution root; binding digest reaches authorization/guard; terminal result produces a retrievable one-file artifact bound to base/scope/bytes | Provider cannot launch from planned/provisioning; parent stays byte-identical; result and artifact recomputation agree; exact one-file/mode/symlink/binary/scope negatives pass |
| P3-P5 — owner integration and cleanup | `pending` | Explicit `worker_integrate` and `worker_cleanup` support only the first one-file overwrite vertical, persist intent/receipt, run separate host verification, close the exact session, officially remove the bound worktree, and independently prove absence | Parent before/after bytes, one apply, idempotent replay, response-loss reconciliation, host-check outcome, Git registration absence, filesystem absence, guard/process/private-home/session cleanup |
| P3-V — installed Worker Broker lifecycle | `pending; qualification 0%` | Installed MCP `worker_spawn_write → wait/result → integrate → restart/replay → cancel → cleanup` with a real provider editing exactly `target.txt` | Preserve exact source/install hashes, worker/operation/session IDs, binding/artifact/integration/cleanup digests, bounded command outcomes, session presence→delete→absence, and immutable receipt. Deterministic tests or direct adapter probes cannot satisfy this gate |
| P3-Q — post-vertical hardening | `blocked_on_P3-V` | Capability receipt probe, recovery breadth, generalized artifact/integration design, fixed Phase 3 producer, final-source replay | Begin only after P3-V; every change maps to an observed live failure or explicit acceptance criterion |
...
| P2.5 | **Source-complete and externally qualified for this slice; final-roadmap replay pending:** producer v4 fixed zero-skip protected Phase 2 producer and exact ordered current Phase 0 plus protected signed Phase 1 predecessor binding; public Phase 2 proof remains rejected; protected operations have no public library exports and execute only through fresh direct-main boundaries; Phase 0 can safely recover after source evolution without accepting malformed history | Exact source `5392d26` passed protected Docker 2/2, internal repository 803/803, 14 hostile scenarios, restart/exact-record replay, full-ledger verification, cleanup, host `npm run check` 803/803, and fresh P0/P1/blocking-P2-clear native review. The installed/authenticated completion/reconnect/cancellation/cleanup vertical passed and its strict receipt is committed at `5b2a58f`. This closes P2.5 only; repeat final live and Phase 0–2 evidence after P3–P5 source freeze |
| P3.1 | Durable private `ExecutionBinding` plus crash-safe exact official-ACP worktree provisioning; dirty control source fails closed and provider executes only in the bound execution root | Contract is frozen in `SPEC.md`: job-first/effect-second admission, exact clean base/fingerprint, canonical control/expected-execution/common-directory identity, scope/role/profile/context binding, fenced `planned → provisioning → ready` journal, no dispatch authority before `ready`, binding digest through every launch boundary, digest-only public projection, and fail-closed adoption/cleanup. Commit `03afdf6` closes the official protocol seam and `65c6fba` closes admission-only planned state. Prove before/after-provision crash recovery, orphan cleanup, symlink/base/control-root mismatch, dirty parent, atomic ready promotion, and zero parent-mutation through the production broker |
| P3-V — mandatory early write vertical | Before broadening Phase 3, one real Grok provider changes exactly one permitted tracked text file in one broker-bound, broker-verified, official-provider-managed worktree. The thin slice crosses only the minimum P3.1 binding/provisioning, P3.3 patch capture, and P3.4 host-owned preview/integration/cleanup needed to obtain real lifecycle feedback. Native code implementation after a concrete Grok worker failure is allowed, but only a real installed Grok provider lifecycle may satisfy this gate | On one exact committed and installed source: MCP admit/spawn → official create/status → independent broker verification → real provider start in the bound worktree → wait → terminal result → one-file artifact/patch retrieval → prove the parent checkout is byte-identical before integration → explicit owner-authorized integration → focused host check → restart/replay plus cancellation/cleanup with no surviving worktree/process/session. Preserve the worker ID, binding/artifact digests, provider observation, parent before/after identities, integration receipt, cleanup evidence, and bounded failure output. Deterministic tests and direct adapter probes support this evidence but cannot replace it |
...
### P3-V early-feedback and go/no-go rule

`P3-V` blocks P3.2, the broad P3.3/P3.4 matrices, P3.5, and all P4–P5 implementation. Implement only enough of P3.1/P3.3/P3.4 to run the vertical above; do not add a Phase 3 producer, managed-root concurrency, generalized artifact formats, or wider presentation work before it passes.
...
12. [In progress: Q1, Q2, and P2.1–P2.5 source boundaries complete; first P2.4 installed authenticated vertical passed; P3 official protocol seam complete at `03afdf6`; P3-P2 durable admission complete at `65c6fba`] Execute P3-P3 through P3-P5 in the pivot table, then run the mandatory installed P3-V real-provider write vertical. Keep the write MCP tool hidden until the distinct live capability and promotion-gated provisioner exist. Do not begin P3.2, the broad P3.3/P3.4 matrices, P3.5, or P4–P5 until P3-V passes; if its bounded remediation cycle fails, mark Phase 3 `blocked_reassess` and simplify or revisit the architecture. After P3-V passes, complete the remaining Phase 3–5 runtime/features and deterministic producers, private aggregate producer/publication boundary, aggregate full-scope, and canonical dual-host artifact binding. Q2's synthetic protected Docker gate and native review do not replace a real final-source issuer attestation. The `e1b03af` provisional v2 receipt proves the first direct-MCP vertical but is superseded by later source changes. Generic writers must remain unable to mint or link qualification. Generate no final phase record or aggregate receipt while these source surfaces are still moving.
