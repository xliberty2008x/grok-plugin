...
- P2.4 is source-complete in the integrated tree, with commit-close and qualification pending: the attempt-bound ordered ACP pump provides body-free inflight state, exact acknowledgement settlement, explicit non-retried `delivery_unknown`, atomic send/close behavior, private/public identity binding, last-turn report binding, and the exact three-capability/ten-tool surface. Focused gates pass (ACP/protocol 26/26, provider plus mailbox 45/45, cancellation/state 19/19, exact full runtime 38/38, installed contract/runner 33/33, v2 evidence 7/7), including generation-1 and report-repair generation-2 cancellation-after-admission barriers 2/2; fresh native rereview is P0/P1-clear. The Grok implementation job failed terminally with `E_PROTOCOL: Internal error`, so the documented native fallback completed the slice. The first exact gate exposed and led to the one-line evidence-fixture remediation above; the new exact committed clean-clone replay and authenticated installed real-provider vertical remain.
