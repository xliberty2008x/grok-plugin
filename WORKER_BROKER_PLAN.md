...
11. [Runner ready; early rehearsal next and non-qualifying] Execute the eight-step direct vertical on the clean plan-bearing commit, inspect/remediate any live defect, then exercise the separate natural-Codex vertical. Mark every resulting receipt provisional/rehearsal-only; any subsequent source change makes it stale, and it cannot qualify the final source.
12. Complete every remaining source-changing prerequisite: the signed independent-review request/import/promote boundary; aggregate-only qualification model; Phase 2–5 runtime/features and fixed deterministic producers; private aggregate producer/publication boundary; aggregate full-scope and canonical dual-host artifact binding. A local subagent report or structural receipt is non-qualifying. Generic writers must remain unable to mint or link qualification. Generate no final record or receipt while these source surfaces are still moving.
