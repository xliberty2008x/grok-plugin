...
| 2 — Mailbox/context/roles | `implemented_unverified` partial production | Idempotent mailbox records, follow-up foundations, explicit ambiguity states, broker-built canonical ContextPacket, immutable explorer role/profile/tool policy, body-free Context Receipt, exact effective-prompt and provider-lineage binding, restart/tamper rejection, legacy-dispatch compatibility, and durable broker-owned host-action requests with owner-only future role grant/deny decisions | Production send has no ACP consumer; follow-up is not dispatchable; ordering, autonomous crash settlement, public role admission/decision operations, and a Phase 2 proof producer are missing |
...
| --- | ---: | --- |
| Full roadmap source implementation | `46%` — `█████████░░░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0; committed Q2 plus source-complete P2.1/P2.2 support 21 of 46 non-deferred deliverables (48 total IDs minus P4-D6/P4-D7 deferred) |
...
| Phase 2 | `5.00 / 8` | ContextPacket/Receipt and exact private host-action request/grant/deny state are production-wired; ACP delivery/follow-up, public decision/admission operations, and the fixed producer remain |
...
| **Total** | **`21.00 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P2 | `D1=0.25, D2=0.5, D3=0.25, D4=1, D5=0.25, D6=0.75, D7=1, D8=1` |
...
| P2-D6 | Immutable explorer/implementer/reviewer/security/test roles | `implemented_unverified` explorer production path plus private future-admission grants; other exact role identities remain unadvertised | RuntimeRolePolicy binds role digest, write bit, exact provider profile/version/agent bytes, and complete allowed/denied provider-tool ID digests; launch preparation rechecks the actual prompt/profile binding; grants derive the exact read-only target policy and never mutate the current worker | Add the public role-admission/follow-up consumption path before reviewer/security/test or implementer is advertised |
| P2-D7 | `awaiting_host_action` authority request | `implemented_unverified` private production path | The final valid provider report may persist one body-free request bound to the exact worker, launch attempt/fence/generation, process/session, context, resume, role, and policy; only the broker-branded exact owner may durably grant/deny idempotently, and public snapshots expose a bounded projection | Expose decision operations atomically with P2.3, consume grants only through exact future admission, and produce final exact-source/live evidence |
...
- P2.3 is next: expose the exact owner decision and future-role admission boundary, then make same-session follow-up enter the normal durable launch outbox with exact grant, parent, resume, context, role, and policy binding.
