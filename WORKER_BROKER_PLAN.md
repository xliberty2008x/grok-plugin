...
| P3.1 | Durable private `ExecutionBinding` plus crash-safe exact detached-worktree provisioning; dirty control source fails closed and provider executes only in the bound execution root | Contract is frozen in `SPEC.md`: job-first/effect-second admission, exact clean base/fingerprint, canonical control/expected-execution/common-directory identity, scope/role/profile/context binding, fenced `planned → provisioning → ready` journal, no dispatch authority before `ready`, binding digest through every launch boundary, digest-only public projection, and fail-closed adoption/cleanup. Prove before/after-provision crash recovery, orphan cleanup, symlink/base/control-root mismatch, dirty parent, and zero parent-mutation |
| P3-V — mandatory early write vertical | Before broadening Phase 3, one real Grok provider changes exactly one permitted tracked text file in one broker-owned detached worktree. The thin slice crosses only the minimum P3.1 binding/provisioning, P3.3 patch capture, and P3.4 host-owned preview/integration/cleanup needed to obtain real lifecycle feedback. No native implementation fallback may satisfy this gate | On one exact committed and installed source: MCP admit/spawn → real provider start in the bound worktree → wait → terminal result → one-file artifact/patch retrieval → prove the parent checkout is byte-identical before integration → explicit owner-authorized integration → focused host check → restart/replay plus cancellation/cleanup with no surviving worktree/process/session. Preserve the worker ID, binding/artifact digests, provider observation, parent before/after identities, integration receipt, cleanup evidence, and bounded failure output. Deterministic tests support this evidence but cannot replace it |
