...
- Exact `56e37e3` closed A-35's code and review requirements and passed validation plus 797/797 with zero failed/cancelled/skipped/TODO, but its authenticated rerun still failed at `completion-result`. Exact diagnostic commit `c5fe559` passed the same gate; after one unrelated fully cleaned setup-command identity-capture race, its clean live retry failed at `completion-result-history-window`, proving A-36's exact retained-tail relation. A-37 keeps the complete sequence-1 stream as independent proof and admits only the canonical max-128 public/private tail; focused tests, fresh exact replay, and the identical live vertical remain mandatory.
