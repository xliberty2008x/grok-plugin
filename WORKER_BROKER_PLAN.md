...
- P2.4 is source-complete through exact commit `f17be92`, with qualification pending: the attempt-bound ordered ACP pump provides body-free inflight state, exact acknowledgement settlement, explicit non-retried `delivery_unknown`, atomic send/close behavior, private/public identity binding, last-turn report binding, and the exact three-capability/ten-tool surface. Focused gates pass (ACP/protocol 26/26, provider plus mailbox 45/45, cancellation/state 19/19, exact full runtime 38/38, installed contract/runner 33/33, v2 evidence 7/7), including generation-1 and report-repair generation-2 cancellation-after-admission barriers 2/2; fresh native rereview is P0/P1-clear. The Grok implementation job failed terminally with `E_PROTOCOL: Internal error`, so the documented native fallback completed the slice. The exact `f17be92` detached clean-clone gate passed validation plus 795/795 with zero failed/cancelled/skipped/TODO. Its first authenticated live attempt reached real provider bootstrap but exposed A-34 in the qualification harness; no receipt was minted, so source completeness remains separate from live qualification.
- The A-34 remediation is included in the commit containing this checkpoint and is deliberately limited to the observed live failure: a normal pre-provider state waits, mailbox open triggers a fresh strict provider/guard read, terminal-before-open fails, and cleanup override diagnostics preserve only fixed allowlisted origin metadata. The targeted runner gate passes 9/9 and repository validation passes in a detached materialization. These checks do not replace the required clean exact-commit replay or the identical authenticated vertical rerun.
