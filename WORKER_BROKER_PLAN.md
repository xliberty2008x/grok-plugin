...
| 2 — Mailbox/context/roles | `implemented_unverified` production path | Provider/session/attempt-bound mailbox capability evidence, an attempt-bound ordered ACP delivery pump, explicit ambiguity states, body-free inflight and communication-chain records, autonomous crash settlement, broker-built canonical ContextPacket, immutable explorer/reviewer/security/test role-profile-tool policies, body-free Context Receipt, exact effective-prompt and provider-lineage binding, restart/tamper rejection, public exact-owner role decisions, and grant-bound same-session follow-up through the normal durable launch outbox | The fixed Phase 2 proof producer, a current exact-source record, and the authenticated installed provider lifecycle are still missing |
...
| --- | ---: | --- |
| Full roadmap source implementation | `51%` — `██████████░░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0; committed Q2 plus source-complete P2.1/P2.2/P2.3 and the integrated P2.4 source boundary support 23.25 of 46 non-deferred deliverables (48 total IDs minus P4-D6/P4-D7 deferred) |
...
| Phase 2 | `7.25 / 8` | ContextPacket/Receipt, exact public host-action decisions, grant-bound same-session follow-up, and the ordered same-session ACP mailbox pump are production-wired; the authenticated installed lifecycle and fixed producer remain |
...
| **Total** | **`23.25 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P2 | `D1=1, D2=1, D3=1, D4=1, D5=0.25, D6=1, D7=1, D8=1` |
...
| Code-owned live qualification receipt | `runner_implemented_unverified` | The v2 runner and private publisher bind the ordered-mailbox lifecycle; generic code still cannot mint or link a pass receipt, and no v2 live receipt exists yet |
| Installed exact-artifact MCP loop | `runner_implemented_unverified` | The runner clean-installs into a private Codex home, verifies byte-identical inventory, validates setup/capability and the exact ten-tool MCP surface, then owns bounded completion/mailbox/replay/cancellation scenarios and cleanup; authenticated execution remains |
| Authenticated provider cancellation and MCP reconnect | `not_started` | One real provider process/session, three ordered prompts with two sends, MCP-server restart without duplicate provider launch, no fourth prompt on replay, idempotent cancellation, and terminal cleanup; this is not worker-crash recovery |
...
- P2.4 is source-complete only at the integrated working-tree level. It remains `implemented_unverified`: authenticated installed real-provider completion/mailbox/replay/cancellation proof, a provisional v2 receipt, and the fixed P2.5 producer/current record are still absent.
...
| A-07 Phase 2 foundation | P2.1–P2.4 production wiring now supplies ContextPacket/Receipt, immutable runtime read roles, exact public owner decisions, grant-bound same-session follow-up through dispatch v2, ordered autonomous ACP mailbox delivery, capability revalidation, nested lineage, transitive retention, and fail-closed replay/recovery | Worker mailbox/context/roles/host-actions/mutation/service/supervisor/MCP files; focused and fake-provider tests | Commit-close P2.4, execute installed/authenticated mailbox and post-terminal/reconnected follow-up proof, then implement the fixed Phase 2 producer/current record |
...
| A-25 Live qualification provenance | The generic evidence model previously accepted caller-authored live-pass claims. Commit `d5f77da` added the v1 fixed-manifest live-receipt boundary; P2.4 adds the v2 ordered-mailbox schema/validator/runner, exact public/private send identity binding, last-turn report binding, direct-versus-natural authority separation, generic-writer/ledger rejection, bounded replay, and no exported mint/build/publish/link authority. This remains an offline structural contract, not proof that a live lifecycle occurred. | Historical v1/P2.3 identities remain immutable; the current v2 manifest digest is `179f210223b5bc963fa94295217fa7ed9710c5f7620ac6e2eb4883f28b943b73`. Schema/runtime/validator parity and tamper regressions pass; no v2 provider pass receipt exists | Run the fixed high-level runner against the current exact-three-capability/ten-tool installed contract, owning setup, inventory, MCP observations, cleanup, and provisional v2 receipt publication end to end. Keep direct MCP provider proof separate from natural Codex installed-host proof |
...
| A-36 Third P2.4 authenticated live attempt | Exact source `56e37e3` implemented A-35 symmetrically and passed validation plus deterministic 797/797, `git show --check`, and clean-tree proof. The identical live rerun again failed `E_PRIVATE_STATE` at `completion-result` after cleanup was proven, so the earlier cleanup-suffix race was real but not the remaining mismatch. Source tracing now narrows the failure to the ordered result observer or its final equality. Production intentionally retains only the newest 128 lifecycle events while the runner accumulates the full cursor stream; existing production coverage permits a public snapshot to begin above sequence 1. | No receipt was minted, no task-owned process survived, and the exact checkout remained clean. Two independent read-only traces rank an exact retained-window mismatch as the dominant remaining cause; result shape/order/content drift remains possible until the live relation is classified. | The diagnostic commit adds no event content, IDs, paths, digests, or provider data. It changes only the fixed failure stage when the returned terminal history is a non-empty exact suffix of the longer tracked history. Pass focused tests and a clean exact gate, rerun the identical live vertical, and change terminal/evidence semantics only if `completion-result-history-window` is observed. Do not slice the tracker or weaken mutation/gap detection |
...
- P2.4 is source-complete through exact commit `d8c0ad3`, with qualification pending: the attempt-bound ordered ACP pump provides body-free inflight state, exact acknowledgement settlement, explicit non-retried `delivery_unknown`, atomic send/close behavior, private/public identity binding, last-turn report binding, and the exact three-capability/ten-tool surface. Focused gates pass (ACP/protocol 26/26, provider plus mailbox 45/45, cancellation/state 19/19, exact full runtime 38/38, installed contract/runner 33/33, v2 evidence 7/7), including generation-1 and report-repair generation-2 cancellation-after-admission barriers 2/2; fresh native rereview is P0/P1-clear. The Grok implementation job failed terminally with `E_PROTOCOL: Internal error`, so the documented native fallback completed the slice. Exact `d8c0ad3` passed validation plus 796/796 with zero failed/cancelled/skipped/TODO and cleared A-34 in the next authenticated attempt, which then exposed A-35 after real terminal wait; no receipt was minted, so source completeness remains separate from live qualification.
- The A-35 remediation is included in the commit containing this checkpoint and is deliberately limited to the observed second live failure: private terminal cleanup is proven before the one public result read, and the strict ordered observer admits only an exact append-only cleanup suffix before final equality. Both completion and cancellation use the same convergence rule. Focused tests, fresh exact replay, and the identical authenticated vertical remain mandatory before this checkpoint can support a receipt.
- Exact `56e37e3` closed A-35's code and review requirements and passed validation plus 797/797 with zero failed/cancelled/skipped/TODO, but its authenticated rerun still failed at `completion-result`; this is A-36, not a successful qualification. The current diagnostic recognizes only a strict retained terminal suffix and reports that relation as the fixed allowlisted `completion-result-history-window` or `cancellation-result-history-window` stage. It does not admit the mismatch or alter evidence. A clean exact replay and live classification remain required.
- P2.5 is next only after the exact P2.4 commit and authenticated live attempt: implement the protected fixed zero-skip Phase 2 producer with exact Phase 0/1 predecessor binding. A live failure must first be classified and rerun against the same vertical; it must not be hidden by producer work.
...
| P2.4 | **Source-complete in the integrated tree, qualification pending:** ordered turn-boundary ACP mailbox pump with attempt-bound atomic open/close state, reserve-then-dispatch RPC correlation, body-free communication-chain binding, acknowledgement-bound `delivered`, non-retried `delivery_unknown`, cancellation-safe primary admission/consumption, and exact live-receipt v2 bindings | Focused gates pass: ACP/protocol 26/26, provider plus mailbox 45/45, cancellation/state 19/19, exact full MCP runtime 38/38, installed contract/runner 33/33, v2 evidence 7/7, and generation-1/generation-2 post-admission cancellation 2/2; validation/diff checks pass; fresh native rereview is P0/P1-clear. The first exact gate exposed one ledger-v1/live-receipt-v2 fixture coupling, now fixed. New exact committed clean-clone `npm run check`, then real-provider happy path, broker reconnect, synchronized inflight termination, no-retry proof, and provisional v2 receipt remain |
...
12. [In progress: Q1, Q2, P2.1, P2.2, P2.3, and the integrated P2.4 source boundary complete; P2.4 commit-close/live attempt then P2.5 next] Complete every remaining source-changing prerequisite: the fixed Phase 2 producer, Phase 3–5 runtime/features and deterministic producers, private aggregate producer/publication boundary, aggregate full-scope, and canonical dual-host artifact binding. Q2's synthetic protected Docker gate and native review do not replace a real final-source issuer attestation. A local subagent report, fake-provider vertical, structural receipt, or provisional v2 live receipt is non-qualifying. Generic writers must remain unable to mint or link qualification. Generate no final phase record or aggregate receipt while these source surfaces are still moving.
