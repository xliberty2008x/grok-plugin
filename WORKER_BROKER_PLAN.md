...
| 2 — Mailbox/context/roles | `implemented_unverified` partial production | Idempotent mailbox records, explicit ambiguity states, broker-built canonical ContextPacket, immutable explorer/reviewer/security/test role-profile-tool policies, body-free Context Receipt, exact effective-prompt and provider-lineage binding, restart/tamper rejection, legacy-dispatch compatibility, public exact-owner role decisions, and grant-bound same-session follow-up through the normal durable launch outbox | Production send still has no ACP consumer; ordered delivery, autonomous crash settlement, a fixed Phase 2 proof producer, post-terminal/reconnected replay proof, and authenticated provider proof are missing |
...
| --- | ---: | --- |
| Full roadmap source implementation | `47%` — `█████████░░░░░░░░░░░` | Transparent rubric: production-wired plus negative-tested = 1, partial production = 0.5, helper/fail-closed scaffold = 0.25, absent = 0; committed Q2 plus source-complete P2.1/P2.2/P2.3 support 21.75 of 46 non-deferred deliverables (48 total IDs minus P4-D6/P4-D7 deferred) |
| First read-only vertical E2E readiness | `89%` — `██████████████████░░` | Eight of nine explicit checkpoints are implemented locally: the code-owned runner now owns exact installation, setup/capability/MCP observations, completion, reconnect/replay/cancellation, cleanup, session deletion, and provisional publication; the authenticated installed provider run remains |
...
| Phase 2 | `5.75 / 8` | ContextPacket/Receipt, exact public host-action decisions, and grant-bound same-session follow-up through the normal durable outbox are production-wired; the ACP delivery pump, post-terminal/reconnected live proof, and fixed producer remain |
...
| **Total** | **`21.75 / 46`** | The two deferred Phase 4 IDs are excluded |
...
| P2 | `D1=0.25, D2=0.5, D3=0.75, D4=1, D5=0.25, D6=1, D7=1, D8=1` |
...
| Code-owned live qualification receipt | `runner_implemented_unverified` | Commit `fda21c2` adds the fixed high-level runner and private publisher; generic code still cannot mint or link a pass receipt, and no live receipt exists yet |
| Installed exact-artifact MCP loop | `runner_implemented_unverified` | Commit `fda21c2` clean-installs into a private Codex home, verifies byte-identical inventory, validates setup/capability and exact MCP identity/tools, then owns both bounded scenarios and cleanup; authenticated execution remains |
...
| 5. Advertise only installed live capabilities | `root-read plus same-session-follow-up subset implemented_unverified`: a private setup receipt binds plugin/MCP contract/provider binary/version/platform/profiles/expiry; the frozen MCP surface exposes six base tools, adds read spawn only for exact root-read capability, and exposes the exact ordered nine-tool inventory only when the receipt also binds same-session follow-up. Root-only, follow-up-only, missing, reordered, duplicated, extra, or drifted capabilities fail back to the base six; send/write remain hidden | Bind the focused negatives to exact-source evidence and prove the same nine-tool set from the installed artifact; ACP acknowledgement and write capabilities remain separate work |
...
The local broker subset, bounded MCP client, stable spawn witness, installed contract, and fixed runner are now present and focused-negative-tested. The remaining proof milestone is to execute that runner through **installed MCP setup receipt → exact nine-tool list → spawn → real provider start → wait → result with `hostVerification:not_run` → cancellation/MCP-reconnect cleanup**, on one exact clean source/install identity. This is an honest root read-only loop, not a positive host-verification receipt. The first follow-up vertical must additionally prove exact same-session resume after terminal state and MCP reconnect. Finding 4 gates the separate write-worker vertical.
...
3. Start the installed MCP server through the bounded client from `0cafff7`; validate the negotiated MCP version/server identity and require the exact ordered nine-tool inventory for the combined root-read and same-session-follow-up receipt. Prove root-only, follow-up-only, missing, reordered, duplicated, extra, and drifted receipts expose only the base six. Call each operation with its own authority `_meta`; prove missing metadata fails with `E_AUTH_REQUIRED`.
...
Producer v3 runs the fixed direct manifests, binds a clean stable source and phase-scope file identity before/after every gate and publication, rejects symlinked scope code plus evidence-only executable seeds/dependencies, closes local static ESM imports with Node's non-evaluating module parser, runs the fixed Phase 1 inventory one file at a time through a structured zero-skip/TODO reporter, stores only bounded redacted output digests, and performs a locked fail-closed baseline cutover. Phase 0 may publish `verified_on_draft` after its exact gates pass. Phase 1 deliberately publishes only `implemented_unverified` with `independentValidation: not_run`: both the runtime validator and published schema reject Phase 1 verified status until authenticated issuer proof exists. The reserved `independentReviewReceipt` checks historical shape/source binding but is not reviewer authentication. A local caller, copied subagent report, or self-computed digest cannot promote Phase 1. Honest promotion requires a code-owned request/import/promote flow plus a signed attestation from a separately protected reviewer issuer, with strict offline signature and exact source/diff/proof replay. After that boundary exists, the fixed Phase 1 producer may emit `verified_on_draft`; Phase 0–5 must still reject `qualified`. Phase 2–5 need their own fixed manifests/runners and deterministic publisher support. A private aggregate producer alone may compose their exact digests with live/corpus/CI evidence and publish `qualified`. The current qualification command is capture-only and exits nonzero when live work is skipped:

Producer v3 is a same-user local qualification runner, not a sandbox or verifier for hostile repository code. Deterministic proof assumes every fixed gate and all descendants are quiescent when the direct gate exits, and that no other process running as the publisher UID concurrently mutates the repository, proof home, or evidence tree. Observable identity mismatch, replacement, inaccessible descendants, cleanup errors, and static symlink escape fail closed before publication. Intentionally surviving same-UID processes, retained open descriptors, witness manipulation, and post-gate mutation require a separately privileged supervisor and are outside this local producer boundary. Successful cleanup proves pathname-level removal of the bound proof-home tree without traversing tested static symlink targets; it does not prove destruction of data retained through an open descriptor by an out-of-contract process. Windows proof production returns typed `E_PROOF_PLATFORM` until an equivalent bound-handle cleanup protocol exists.
...
| P0-D4 | Proof-producing capture/replay workflow | `implemented_unverified` | Producer v3; aggregate-only qualification semantics; fixed direct Phase 0/1 manifests; exact serial Phase 1 runner; zero-skip reporter; scope-file identity, symlink, and executable-evidence-path defense; broker-owned provenance; absolute toolchain binding; private proof environment/promotion authority; immutable signed review request/attestation; root-owned protected Ed25519 trust/runtime/Git boundary; atomic Phase 1 promotion; safe cutover and strict ledger replay. Current draft evidence: 121/121 evidence tests, external Docker 1/1 with restart and 14 hostile cases, and independent P0/P1-clear rereview | Commit Q2, continue source-changing phases, then freeze once. On the final exact source publish Phase 0/1 proofs, obtain a real protected independent issuer attestation, run protected promotion, and persist the exact request/attestation/receipt/record/ledger proof. The synthetic Docker signer is supporting evidence only |
...
| P2-D3 | Active send and terminal/idle lineage follow-up | `implemented_unverified` partial production | Exact-owner grant-bound follow-up commits one child through dispatch v2, resumes the exact parent provider session, preserves root lineage and immediate parent identity, freezes child-bound final context, and replays without a second launch/prompt; a fake-provider MCP vertical proves one `session/new`, one exact `session/load`, and two prompts in one session | Add the worker-owned ACP safe-boundary pump for active send, then prove post-terminal/reconnected follow-up and the same-session chain through the real installed provider |
...
- The authoritative closing check is a detached clean clone of the exact commit running `npm run check`, followed by `git show --check --format= HEAD`. Record that commit and exact zero-fail/zero-skip counts in issue #25 and PR #26; if either command fails, P2.3 reopens.
- None of these local checks creates lifecycle qualification. Phase 2 still requires the fixed producer/current record plus an installed authenticated nine-tool decision → follow-up run, including child terminal state, MCP reconnect, replay, and proof that no third prompt is sent.
...
| A-07 Phase 2 foundation | P2.1–P2.3 production wiring now supplies ContextPacket/Receipt, immutable runtime read roles, exact public owner decisions, grant-bound same-session follow-up through dispatch v2, capability revalidation, nested lineage, transitive retention, and fail-closed replay/recovery | Worker mailbox/context/roles/host-actions/mutation/service/supervisor/MCP files; focused and fake-provider tests | Implement ordered autonomous ACP mailbox delivery, the fixed Phase 2 producer, post-terminal/reconnected follow-up proof, and final installed/authenticated live evidence |
...
| A-09 Phase 4 foundation | Raw task-owned MCP operations, immutable exact-capability-gated spawn/decision/follow-up advertisement, external labeling, structured-proof suppression, and normalized host-attested summary/progress/event suppression exist; installed flows still lack the final natural-host qualification path | Provider capability, worker presentation/protocol/service/MCP tests, and host scripts; P2.3 adds exact-six/exact-nine gating and hostile-input regressions | Build native-shaped presentation/multi-wait, persistence, positive broker-owned verification, MCP-first skill routing, installed natural Codex/Claude flows, and a Phase 4 producer |
...
| A-20 Source-backed pre-E2E audit delta | Issue comment 5048315213 supplied seven commit-pinned blockers and three post-first-vertical improvements; P2.1–P2.3 close the context/lineage/public-decision/same-session-follow-up source subsets while preserving unsupported-tool hiding | Current source, acceptance mapping, P2.3 fake-provider vertical, and this plan's progress rubric | Implement the ACP mailbox pump, execute authenticated root/follow-up and later write verticals separately, and add P2-D8/P4-D8/P5-D8 gates before aggregate qualification |
...
| A-25 Live qualification provenance | The generic evidence model previously accepted caller-authored live-pass claims. Commit `d5f77da` adds a separate fixed-manifest live-receipt schema/validator, direct digest/install-method/runtime binding, direct-versus-natural authority separation, bidirectional provisional semantics, generic-writer/ledger rejection, bounded directory replay, and no exported mint/build/publish/link authority. P2.3 extends the synthetic installed contract to the exact nine-tool combined-capability inventory. This remains an offline structural contract, not proof that a live lifecycle occurred. | Exact `d5f77da` originally bound manifest digest `0641afd02156262492b10f52dd71f4ce18aa6b338179324a34d6b2b111b6370b`; the P2.3 nine-tool manifest digest is `aee11910fb0ff30ac4d7475fbd29c79cc07fed679e037b602d383a94e7694833`. Schema/runtime/validator parity and tamper regressions pass; no final-source provider pass receipt exists | Run the fixed high-level runner against the current nine-tool installed contract, owning setup, inventory, MCP observations, cleanup, and provisional receipt publication end to end. Keep direct MCP provider proof separate from natural Codex installed-host proof |
...
| A-30 Installed-runner source map | Read-only source mapping established the exact installed setup/receipt/tool APIs and the private persisted-state checks needed for lifecycle proof. It also corrected two tempting false proofs: active-provider cancellation receipts keep `processGroupGoneAt`/`terminalRecordCommittedAt` null because they are immutable admission receipts, and `isImportedSessionReady()===false` cannot distinguish successful deletion from list failure. The mapped runner is now implemented in `fda21c2`. | Installed wrapper/setup, provider-capability receipt, exact tools order, state/mutation/guard/process/session modules, cancellation/session tests, and the fixed runner/contract | Execute the committed runner against the authenticated installed provider; inspect its provisional receipt and cleanup evidence before treating this audit item as live-proven |
| A-31 Completion-model/producer audit | Fixed manifests exist only for Phase 0/1; CLI proof accepts only those phases; Phase 2–5 dependent publication/status recognition is internally inconsistent; aggregate has no producer, real scope seed, or protected publication authority; Worker Broker aggregate and SPEC's canonical dual-host qualification artifact are not yet reconciled | Exact source audit of evidence schema/library/CLI, plan, `SPEC.md`, version policy, and validation; no files or evidence were mutated | Add Phase 2–5 manifests/runners at a bumped producer/manifest version, signed Phase 1 promotion, aggregate full-scope binding, private aggregate publication, corrected `--require-complete`, historical non-qualification, and dual-host artifact reconciliation. Prove generic writers cannot mint/link aggregate qualification |
| A-32 Stable spawn response and installed runner | Commit `858ceea` returns the exact transaction-time untrusted admission handle and records a private schema-4 response witness instead of replacing the response with a moving post-dispatch reread. Commit `fda21c2` adds exact witness/launch-contract/public-private binding plus the fixed installed completion and reconnect/replay/cancellation runner; P2.3 updates its contract to exact-six/exact-nine capability gating. | Earlier pre-commit integrated checks passed on current Node and Node 18.18.2; P2.3 installed-contract/tool-gating, hostile-input, and fake-provider follow-up tests pass. These moving-tree checks do not replace the clean exact-source replay. | Run the clean committed full repository gate, then execute the authenticated installed nine-tool runner. Keep the receipt provisional because later Phase 2–5 source work will supersede it |
...
- Q1 is complete in the commit containing this checkpoint: producer v3 reserves `qualified` for one aggregate record, requires six current `verified_on_draft` numbered phases plus that aggregate for `--require-complete`, binds aggregate scope to the complete non-evidence source inventory, and rejects generic aggregate mint/link/prove paths. The final focused gate passed 111/111 and repository validation passed.
...
- P2.3 is source-complete in the commit containing this checkpoint, with qualification pending: `worker_decide_host_action` exposes only the bounded exact-owner decision input/output; `worker_followup` consumes one exact grant and commits a reviewer/security/test child through the normal dispatch-v2 outbox with immediate-parent resume, root lineage/provider-home identity, exact same-session load, child-bound final context, capability revalidation, transitive retention, and fail-closed replay/recovery. The final focused gate passed 204/204 and the fake-provider MCP vertical passed 1/1 with one session creation, one exact session load, two prompts, and no replayed third prompt; a fresh native review found no P0/P1. The Grok implementation job failed terminally with `E_PROTOCOL`, so the documented native fallback completed the bounded slice. The exact clean-clone repository replay remains the commit-closing check, and this is not installed authenticated proof.
- P2.4 is next: add the ordered turn-boundary ACP mailbox pump with acknowledgement-bound `delivered`, body-free inflight state, explicit non-retried `delivery_unknown`, and atomic close/empty-scan behavior.
...
| P2.3 | **Source-complete, qualification pending:** public exact-owner decision plus grant-bound reviewer/security/test follow-up enters the normal dispatch-v2 outbox, resumes exactly one parent provider session, preserves immediate-parent/root lineage, and freezes child-bound final context | Frozen-tree production batch 204/204; P1 regressions 50/50; fake-provider MCP vertical 1/1 proving one `session/new`, one exact `session/load`, two same-session prompts, and no replayed third prompt; capability/authority/context/profile/session/dispatch/retention/recovery/privacy negatives; validation/diff checks pass; fresh native P0/P1-clear review. Clean-clone exact-commit `npm run check` plus installed authenticated post-terminal/reconnected replay remain required |
| P2.4 | Ordered turn-boundary ACP mailbox pump with attempt-bound atomic open/close state, reserve-then-dispatch RPC correlation, body-free communication-chain binding, acknowledgement-bound `delivered`, and non-retried `delivery_unknown` | Same-lock send/empty-close race; cumulative 32-message/256-KiB bounds; two-message fake vertical on one process/session with three ordered prompts and last-turn report; wrong/colliding/malformed/duplicate/error response rejection; every inflight crash/cancel/recovery window; unknown blocks later turns; no body remains; report repair cannot reopen; exact three-capability/ten-tool gating; then real-provider happy path, broker reconnect, synchronized inflight termination, and no-retry proof |
| P2.5 | Fixed zero-skip Phase 2 producer and exact Phase 0/1 predecessor binding | Static-import/scope drift, skip/TODO, missing predecessor, atomic publication, and strict replay tests |
...
12. [In progress: Q1, Q2, P2.1, P2.2, and P2.3 source boundaries complete; P2.4 next] Complete every remaining source-changing prerequisite: Phase 2–5 runtime/features and fixed deterministic producers; private aggregate producer/publication boundary; aggregate full-scope and canonical dual-host artifact binding. Q2's synthetic protected Docker gate and native review do not replace a real final-source issuer attestation. A local subagent report, fake-provider vertical, or structural receipt is non-qualifying. Generic writers must remain unable to mint or link qualification. Generate no final record or receipt while these source surfaces are still moving.
