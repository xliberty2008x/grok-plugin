...
- Integrate the structured result and run independent host verification. The one-shot `record-verification <job-id> --verification-stdin` accepts at most 64 KiB and only a root `commandOutcomes` array of 1 through 64 unique entries. Each entry contains only an exact declared `command`, `status` (`passed` or `failed`), and integer `exitCode`; no stdout, stderr, output, or summary fields. Passing records cover every distinct required command with passed/0; failing records may be partial but contain a failed status or nonzero code. Treat the resulting scope-checked checkpoint as `host_asserted`, not as proof of command-to-diff causality. Declare expected ignored verification outputs in TaskEnvelope `verificationGeneratedPaths` (or isolate them outside the workspace); undeclared `dist/**` / `node_modules/**` drift still fails ordinary completion and resume.
