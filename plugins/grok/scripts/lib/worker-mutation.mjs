...
export function admitWriteWorkerPlan({
  ...
  // Fail closed on unsafe index flags/structures before ContextManifest identity (which now observes those flags) can preempt with E_CONTEXT_DRIFT.
  captureParentFingerprint(control.controlRoot);
  // Caller-provided admission captures are integrity-checked once up front. Fresh capture for new admissions stays deferred until the no-replay path so same-key replay can rebind exclusively to the job's stored admission IDs.
