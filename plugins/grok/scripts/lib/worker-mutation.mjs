...
const WRITE_PROVISIONING_RUNTIME_KEYS = new Set([
  "schemaVersion",
  "intent",
...
const OFFICIAL_WORKTREE_RECEIPT_INPUT_KEYS = new Set([
  "status",
  "sessionId",
  "worktreePath",
  "sourceGitRoot",
...
const OFFICIAL_WORKTREE_RECEIPT_KEYS = new Set([
  "schemaVersion",
  "operationId",
  "officialStatus",
  "officialSessionId",
...
const WRITE_PROVISIONING_CLEANUP_KEYS = new Set([
  "schemaVersion",
  "providerSpawnIntentId",
  "processIdentity",
  "processGroupGone",
...
const WRITE_PROVISIONING_PURPOSE = "worktree-provisioning";
const WRITE_PROVISIONING_SCHEMA_VERSION = 1;
const WRITE_PROVISIONING_INTENT_STATUSES = new Set([
  "pending",
  "registered",
  "settled",
  "no-child"
]);
const WRITE_PROVISIONING_NO_CHILD_RESOLUTIONS = new Set([
  "spawn-not-created",
  "cleanup-proven",
...
function writeProvisioningStateError(message, code = "E_STATE") {
...
function assertTimestampNotBefore(value, minimum, label) {
...
function assertBoundedOperationId(value) {
...
function assertWriteProvisioningProcessIdentity(identity, {
  nullable = false
} = {}) {
...
function sameWriteProvisioningProcessIdentity(left, right) {
  try {
...
function writeProvisioningIntentDigestBody(intent) {
  return {
...
function worktreeVerificationWithoutDigest(verification) {
  const { verificationDigest: _verificationDigest, ...body } = verification;
  return body;
}

function assertWorktreeHostVerification(verification, binding) {
...
function officialWorktreeReceiptWithoutDigest(receipt) {
  const { receiptDigest: _receiptDigest, ...body } = receipt;
  return body;
}

function assertOfficialWorktreeReceipt(receipt, binding, intent) {
...
function assertWriteProvisioningCleanupProof(proof, intent, {
  preactivation = (
    intent?.status === "no-child"
...
export function admitWriteWorkerPlan({
  ...
  const admitted = withWorkspaceStateTransaction(control.controlRoot, function admitWritePlanTransaction(transaction) {
    ...
    if (existing) {
      ...
      const committed = transaction.tryReadJob(
        record.workerId
      );
      ...
      if (record.admissionRequestDigest
        !== committed.request.spawn.admissionRequestDigest) {
...
export function spawnReadOnlyWorker({
  ...
  const admitted = withWorkspaceStateTransaction(root, (transaction) => {
    ...
    if (existing) {
      ...
      assertDurableSpawnRequestBinding(committed, env, { contextPhase: "admission" });
      assertMutationOwnership(committed, principal);
    ...
    if (orphan) {
      ...
      assertDurableSpawnRequestBinding(orphan, env, { contextPhase: "admission" });
      assertMutationOwnership(orphan, principal);
      const replayRequestDigest = storedSpawnReplayRequestDigest({
        job: orphan,
        principal,
...
function captureManagedWritePostBindingContext(
  ...
  if (observedContextManifest == null) {
    try {
      currentContextManifest = captureContextManifest(
        binding.expectedExecutionRoot
      );
    } catch {
      throw contextIncompleteError("terminal", ["contextCapture"]);
    }
    currentContextManifest = assertContextManifestIntegrity(
      currentContextManifest
    );
  } else {
    currentContextManifest = assertContextManifestIntegrity(
      observedContextManifest
    );
  }
